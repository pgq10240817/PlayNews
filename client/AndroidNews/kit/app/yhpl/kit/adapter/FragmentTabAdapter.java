...
public class FragmentTabAdapter<T extends Fragment> implements RadioGroup.OnCheckedChangeListener {
	...
	public FragmentTabAdapter(FragmentManager fragmentManager, List<T> fragments, int fragmentContentId,
			RadioGroup rgs, OnTabDoubleClickListener onTabDoubleClickListener) {
	...
	public void onCheckedChanged(RadioGroup radioGroup, int checkedId) {
		for (int i = 0; i < rgs.getChildCount(); i++) {
			if (rgs.getChildAt(i).getId() == checkedId) {
				if (ABApplication.getInstance().getString(R.string.tag_fragment_tab_click_only)
						.equals(rgs.getChildAt(i).getTag())) {
