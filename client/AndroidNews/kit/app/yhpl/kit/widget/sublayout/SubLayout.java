...
import app.yhpl.kit.manager.OnActivityLifeCycleFullListener;
import app.yhpl.kit.mvp.ABActivityViewer;
import app.yhpl.kit.mvp.ABBasePresenter;
import app.yhpl.kit.present.ABActivityCommon;

/**
 * Created with IntelliJ IDEA. User: wangjie Date: 14-4-24 Time: 下午6:12 To
...
public class SubLayout implements ISubLayout, ABActivityViewer, OnActivityLifeCycleFullListener {
	...
	public void showInfoDialog(String title, String message, String okButtonText) {
		new AlertDialog.Builder(context).setTitle(title).setMessage(message).setPositiveButton(okButtonText, null)
				.show();
	}

	/**
	 * 绑定Activity的生命周期（需要调用ctivity.getBaseActivityManager().
	 * registerOnActivityLifeCycleListeners(this);）
	 */
	@Override
