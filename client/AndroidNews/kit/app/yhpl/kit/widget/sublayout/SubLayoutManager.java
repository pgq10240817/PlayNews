package app.yhpl.kit.widget.sublayout;
...
import app.yhpl.kit.manager.OnActivityLifeCycleListener;
import app.yhpl.kit.objs.DelayObj;
...
public class SubLayoutManager<T extends SubLayout> {
	...
	/**
	 * 获取需要替换的ViewGroup <FrameLayout android:id="@+id/main_content_view"
	 * android:layout_width="match_parent" android:layout_height="0dp"
	 * android:layout_weight="1.0" /> contentView = (ViewGroup)
	 * findViewById(R.id.main_content_view); 然后通过构造方法传入
	 *
	 * @return
	 */
	...
	public void switchLayout(int position) {
		...
		try {
			...
			if (null == subLayout.getLayout()) { // 如果layout为空，即setContentView方法未调用
				Logger.e(TAG, subLayout.getClass().getSimpleName()
						+ "'s layout is null！！please invoke setContentView method...");
				return;
			}
			...
			contentView.addView(subLayout.getLayout(), new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
