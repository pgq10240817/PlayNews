package app.yhpl.kit.widget.horizontialmenu;
...
public class HorizontialComplexMenu implements View.OnClickListener {
	...
	public static PopupWindow createHoriMenu(Context context, Drawable background, List<HorizontialMenuItem> items,
			OnHorizontialComplexMenuListener listener) {
		HorizontialComplexMenu menu = new HorizontialComplexMenu(context, background, items, listener);
		return menu.popMenu;
	}

	public HorizontialComplexMenu(Context context, Drawable background, List<HorizontialMenuItem> items,
			OnHorizontialComplexMenuListener listener) {
	...
	private void initMenu() {
		...
		for (int i = 0; i < itemSize; i++) {
			...
			im.setPadding(ABTextUtil.dip2px(context, 8), ABTextUtil.dip2px(context, 3), ABTextUtil.dip2px(context, 8),
					ABTextUtil.dip2px(context, 3));
			im.setTag(i);
			if (null != listener) {
		...
		popMenu.setBackgroundDrawable(null == background ? new ColorDrawable(Color.parseColor("#cc000000"))
				: background);
	...
	public static class HorizontialMenuItem implements Serializable {
		...
		public String toString() {
			return "SheetItem{" + "title='" + title + '\'' + ", textColor=" + textColor + ", textSizeSp=" + textSizeSp
					+ ", action=" + action + '}';
		}
