...
import app.yhpl.kit.utils.ABTimeUtil;
...
public class Logger {
			...
			public Void runInBackground() {
				synchronized (Logger.class) {
					...
					String filename = logFilePath
							+ ABTimeUtil.millisToStringFilename(System.currentTimeMillis(), "yyyy-MM-dd") + ".log";
					...
					try {
						...
						String formatMsg = ABTimeUtil.millisToStringDate(System.currentTimeMillis())
								...
								"Throwable Message: " + tr.getMessage() + "\r\n" + "Throwable StackTrace: "
										+ transformStackTrace(tr.getStackTrace())) + "\r\n";
	 ...
	 */
	/*
	 * static class WriteLogThread extends Thread{ private String tag; private
	 * String msg; private Throwable tr; private String priority;
	 * 
	 * WriteLogThread(String tag, String msg, Throwable tr, String priority) {
	 * this.tag = tag; this.msg = msg; this.tr = tr; this.priority = priority; }
	 * 
	 * @Override public void run() { synchronized (WriteLogThread.class){
	 * 
	 * String filename = logFilePath + File.separator +
	 * ABTimeUtil.millisToStringFilename(System.currentTimeMillis(),
	 * "yyyy-MM-dd") + ".log"; File logFile = new File(filename);
	 * 
	 * OutputStream os = null; try{ if(!logFile.exists()){
	 * logFile.createNewFile(); }
	 * 
	 * os = new FileOutputStream(logFile, true);
	 * 
	 * String formatMsg =
	 * ABTimeUtil.millisToStringDate(System.currentTimeMillis()) + "\r\n[" +
	 * priority + "][" + tag + "]: \r\n" + "User Message: " + msg + "\r\n" +
	 * (null == tr ? "" :
	 * 
	 * "Throwable Message: " + tr.getMessage() + "\r\n" +
	 ...
	 * "\r\n";
	 * 
	 * os.write(formatMsg.getBytes("utf-8")); os.flush(); }catch(Exception ex){
	 * ex.printStackTrace(); return; }finally{ if(null != os){ try { os.close();
	...
	/*
	 * private static synchronized void writeToLogFile(String tag, String msg,
	 * Throwable tr, String priority) { String filename =
	 * Variables.APP_LOG_SDPATH + File.separator +
	 * ABTimeUtil.millisToStringFilename(System.currentTimeMillis(),
	 * "yyyy-MM-dd") + ".log"; File logFile = new File(filename);
	 * 
	 * OutputStream os = null; try{ if(!logFile.exists()){
