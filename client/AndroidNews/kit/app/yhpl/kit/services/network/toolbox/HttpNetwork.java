...
public class HttpNetwork implements Network<HippoHttpRequest<?>> {
	...
	private static void setEntityIfNonEmptyBody(HttpEntityEnclosingRequestBase httpRequest, HippoHttpRequest<?> request) {
		...
		while (true) {
			...
			try {
				...
				if (httpResponse != null) {
					...
					if (responseContents != null) {
						networkResponse = new NetworkResponse(new HippoException("Error status code: " + statusCode),
								responseContents);
					}
				} else {
	...
	private static void attemptRetryOnException(HippoHttpRequest<?> request, HippoException exception)
