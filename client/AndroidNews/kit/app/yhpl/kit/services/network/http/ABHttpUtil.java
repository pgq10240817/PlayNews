package app.yhpl.kit.services.network.http;
...
import app.yhpl.kit.services.network.http.interceptor.HttpMethodInterceptor;
...
public class ABHttpUtil {
	...
	public static HttpClient getSSLHttpClient(HttpConfig httpConfig) {
		try {
			...
			int[] ports;
			if (null == onHttpSessionConnectListener || null == (ports = onHttpSessionConnectListener.getPorts())
					|| ports.length < 2) {
	 ...
	 * @return
	 */
	public static String getHttpResponse(HttpAccessParameter accessParameter, int soTimeout, int connectionTimeout)
			throws Exception {
		...
		try {
			// 将流转换为字符串
			HttpResponse httpResponse = getHttpResponse(soTimeout, connectionTimeout, httpRequest, url);
	...
	private static HttpResponse getHttpResponse(int soTimeout, int connectionTimeout, HttpUriRequest httpRequest,
	...
	/**
	 * 根据是否根据SessionMethod来构造访问URL 如果使用SessionEnableMethod.ENABLE则启用Session访问
	...
	private static String generateUrl(HttpAccessParameter accessParameter) throws Exception {
		String url = null == onHttpSessionConnectListener ? httpConfig.getDomain() : onHttpSessionConnectListener
				.getDomain();
		if (isEnableSSL(url)) {
			url += ":"
					+ (null == onHttpSessionConnectListener ? httpConfig.getHttpsPort() : onHttpSessionConnectListener
							.getPorts()[1]);
		} else {
			url += ":"
					+ (null == onHttpSessionConnectListener ? httpConfig.getHttpPort() : onHttpSessionConnectListener
							.getPorts()[0]);
		}
